...
<../apps/Witness>` (for monitoring only) or the :doc:`CLI wallet
<../apps/CliWallet>` (for accessing funds). Instead of running the P2P full
node and the CLI wallet separately, we also offer a :doc:`combined executable
<../apps/CliFullWallet>` that perform both tasks simultaneously.
